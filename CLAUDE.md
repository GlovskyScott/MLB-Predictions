...
- **Rebuild after a retrain:** `python -m scripts.build_calibrator` (walk-forward; ~1–2 min). It builds **both** calibrators — `model_calibrator.pkl` (win%) and `model_inning_dist_calibrator.pkl` (the 3-class per-inning P(0/1/2+) calibrator). Both are model artifacts in the `latest` release (restored best-effort by `bootstrap_model_cache`), **not** in `_MODEL_PKLS` and **not** in the version hash.
- **Inning model & calibration:** the inning classifier is **3-class** (P(0 runs) / P(1) / P(2+) per inning per team; `model.runs_to_bucket` labels it, `train_inning_model` is `multi:softprob`). `predict_inning_probs` returns `home/away_innings_dist` (9×3) plus `combined_innings_dist` (P(any team scores), via `_combine_inning_dist` under home/away independence). `_calibrate_core` calibrates the per-team dists with a one-vs-rest `MulticlassInningCalibrator` and recomputes the combined row; the map is near-identity (the classifier is already well-calibrated, slope ≈ 0.97). The simulator emits the same three fields from its inning matrix as a **fallback** (a legacy 2-class pkl makes `predict_inning_probs` raise, so the sim's dist is kept). The win% map, by contrast, is a hard shrink (slope ≈ 0.48). Retargeting the inning model changes `model_inning.pkl` → **forks a new version**.
- **Moneyline only.** The app supports the **moneyline market only**. The run-line (spread) and total (O/U) markets were removed: their edges were computed from the raw, *uncalibrated* score distribution, and the run line additionally against a fabricated 50% baseline (ESPN exposes the ±1.5 line but no per-side run-line price), which manufactured implausible 15–31% "edges". `compare_date`/`_version_summary` expose `ml_accuracy` (== `winner_accuracy`); there is no `spread_accuracy`/`total_accuracy`, no `src/grading.py`, and no `data/market_lines/` snapshot store anymore. The ML edge in `_market_block` is the **calibrated** win% vs the de-vigged market moneyline.
- **Market-blended "Consensus" line (headline + graded).** The displayed and graded moneyline is `sigmoid(a·logit(model_raw) + b·logit(market_devig) + c)` from `src/blend.py` (`model_market_blender.pkl`), fit walk-forward by `scripts/build_calibrator.py` on `(raw model prob, de-vigged market prob, outcome)` triples. Unlike the win% calibrator it is **not pick-preserving** — it can move the favored side toward the market — so the blended line is the **prediction of record**: `_blend_core` (called after `_calibrate_core` in both `run_daily_simulation` and `compare_date`) overwrites `home_win_pct`/`away_win_pct` and keeps the model-only value under `model_*`. Grading reports `consensus_accuracy` (== `winner_accuracy`/`ml_accuracy`, the headline) and `model_accuracy` (model-only, for comparison). Inputs: the write-once moneyline store `data/market_odds/<date>.json` (`predictions.save/load_market_odds`, captured live in `run_daily_simulation`, backfilled by `scripts/backfill_market_odds.py` from ESPN closing lines; `fetcher.devig_home_prob` removes the ~20-cent hold and reconstructs a one-sided quote on a standard 20-cent line). Serve-time only (no fork); falls back to the calibrated model line when a game has no market line or the blender is absent. Rebuild order after a retrain: `backfill_market_odds.py` → `build_calibrator.py` (fits both calibrators **and** the blender).
- **Scope:** the moneyline/win% and the per-inning run distribution are calibrated; the headline moneyline is additionally market-blended.

## Conventions & gotchas

- **Starter rate stats are regressed to league average by IP (v5).** `features._shrink_pitcher_stats` applies `shrunk = (ip·raw + K·prior)/(ip + K)` (K = `_PITCHER_SHRINK_IP` = 50, prior = `_PITCHER_DEFAULTS`) to ERA/FIP/xFIP/WHIP/K9/BB9/HR9, after any home/away splits, in **both** the training and inference paths. Without it a tiny-sample starter (e.g. 3 IP, 12.00 FIP) is taken at face value and the run regressors blow up the opponent's predicted runs. `ip == 0` (TBD/no-data) → exactly the league-average prior. Re-tune K via `scripts/tune_regressors.py` if you touch this.
