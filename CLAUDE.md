...
- **Market accuracy (ML/Total/Spread):** grading scores three markets — ML (favorite wins), Spread (run-line ±1.5 cover), Total (O/U vs the captured market line). `src/grading.py` derives the Spread/Total picks from the frozen core's marginal `score_distribution` by convolution (no re-sim). The market total line is snapshotted **write-once** per date in `data/market_lines/<date>.json` (version-independent, captured live from `get_market_odds`); Total is N/A for dates with no captured line. `compare_date`/`_version_summary` expose `ml/spread/total_accuracy`.
- **Scope:** the moneyline/win% and the per-inning run distribution are calibrated. The run-line and total *edges* still derive from the raw score distribution — calibrating those is a separate follow-up.
